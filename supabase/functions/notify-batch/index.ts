// Sends a Web Push to everyone granted the matching section when a new batch
// of work lands — a Daily Pick List sheet or a Bundle SKU order. Same
// broadcast shape as notify-pickup: the app calls this itself right after it
// writes, so there is no database webhook to configure, and the section's
// permission column is the guest list, same as notify-pickup's can_pickup.
//
...
// which section's permission column gets notified, and what the banner says
const KIND: Record<string, { col: string; title: string }> = {
  picklist: { col: "can_daily_pick_list", title: "New pick list" },
  bundle: { col: "can_bundle_sku", title: "New bundle order" },
};
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { kind, title, from, test } = await req.json();
    const spec = KIND[kind];
    if (!spec) return json({ error: "unknown kind" }, 400);

    // Everyone granted the section works the same batch, so a new one has to
    ...
    // the section's permission alone, matching the database rule. Being an
    // admin does not put you on the list — otherwise switching the section
    // off for someone would leave their phone still buzzing.
    const { data: team, error: teamErr } = await admin
      .from("profiles").select("user_id").eq(spec.col, true);
    ...
    // the phone that uploaded the batch already has it open; do not buzz it
    // again — unless this is someone checking their own setup works
